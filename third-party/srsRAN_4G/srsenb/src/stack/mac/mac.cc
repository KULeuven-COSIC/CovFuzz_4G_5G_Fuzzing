...
int mac::get_dl_sched(uint32_t tti_tx_dl, dl_sched_list_t& dl_sched_res_list)
{
  ...
  for (uint32_t enb_cc_idx = 0; enb_cc_idx < cell_config.size(); enb_cc_idx++) {
      ...
      if (ue_db.contains(rnti)) {
        ...
        for (uint32_t tb = 0; tb < SRSRAN_MAX_TB; tb++) {
          ...
          if (sched_result.data[i].nof_pdu_elems[tb] > 0) {
            ...
            // FUZZ START: DL MAC INTERCEPTION
            if (fuzz_config_g.FUZZ_MAC_DL) {
                    printf("Interception point 1\n");
                    shm_helper_g.send_mac_msg(T_ENB_MAC_UE_DL_PDU_WITH_DATA,
                          DIRECTION_DOWNLINK,
                          WS_C_RNTI,
                          rnti,
                          (uint16_t)(tti_tx_dl/10),
                          (uint16_t)(tti_tx_dl%10),
                          dl_sched_res->pdsch[n].data[tb],
                          sched_result.data[i].tbs[tb],
                          -1);
            }
            // FUZZ END: DL MAC INTERCEPTION
    ...
    for (uint32_t i = 0; i < sched_result.rar.size(); i++) {
      ...
      // FUZZ START: DL MAC INTERCEPTION
      if (fuzz_config_g.FUZZ_MAC_DL) {
          printf("Interception point 2\n");
        	shm_helper_g.send_mac_msg(T_ENB_MAC_UE_DL_RAR_PDU_WITH_DATA,
                  DIRECTION_DOWNLINK,
                  WS_RA_RNTI,
    ...
    for (uint32_t i = 0; i < sched_result.po.size(); i++) {
      ...
      if (ue_db.contains(rnti)) {
        ...
        // FUZZ START: DL MAC INTERCEPTION
        if (fuzz_config_g.FUZZ_MAC_DL) {
            printf("Interception point 3\n");
          	uint16_t rnti = sched_result.bc[i].dci.rnti;
            shm_helper_g.send_mac_msg(T_ENB_MAC_UE_DL_PDU_WITH_DATA,
                      DIRECTION_DOWNLINK,
                      WS_C_RNTI,
                      rnti,
      ...
      if (sched_result.pusch[i].tbs > 0) {
        ...
        if (ue_db.contains(rnti)) {
          ...
          if (phy_ul_sched_res->pusch[n].data) {
            ...
            // FUZZ START: DL MAC INTERCEPTION
            if (fuzz_config_g.FUZZ_MAC_DL) {
                printf("Interception point 4\n");
                shm_helper_g.send_mac_msg(T_ENB_MAC_UE_UL_PDU_WITH_DATA,
                      DIRECTION_UPLINK,
                      WS_C_RNTI,
                      rnti,
                      (uint16_t)(tti_tx_ul/10),
                      (uint16_t)(tti_tx_ul%10),
                      phy_ul_sched_res->pusch[n].data,
                      sched_result.pusch[i].tbs,
                      -1);
            }
            // FUZZ END: DL MAC INTERCEPTION
