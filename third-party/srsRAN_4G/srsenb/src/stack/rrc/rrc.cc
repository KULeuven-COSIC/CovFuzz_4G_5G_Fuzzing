...
void rrc::parse_ul_ccch(ue& ue, srsran::unique_byte_buffer_t pdu)
{
  ...
  // FUZZ START: UL CCCH INTERCEPTION
  if (fuzz_config_g.FUZZ_RRC_DL) {
    shm_helper_g.send_rrc_msg(DIRECTION_UPLINK, CHANNEL::UL_CCCH, reinterpret_cast<uint8_t*>(pdu->msg), pdu->N_bytes);
  }
  // FUZZ END: UL CCCH INTERCEPTION
