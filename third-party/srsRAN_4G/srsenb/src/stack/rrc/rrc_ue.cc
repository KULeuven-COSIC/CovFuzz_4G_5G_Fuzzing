...
void rrc::ue::parse_ul_dcch(uint32_t lcid, srsran::unique_byte_buffer_t pdu)
{
  ...
  // FUZZ START: UL DCCH INTERCEPTION
  if (fuzz_config_g.FUZZ_RRC_DL) {
    shm_helper_g.send_rrc_msg(DIRECTION_UPLINK, CHANNEL::UL_DCCH, reinterpret_cast<uint8_t*>(pdu->msg), pdu->N_bytes);
  }
  // FUZZ END: UL DCCH INTERCEPTION
