                                ...
                                const nas_if_t&         itf)
{
  ...
  // FUZZ: ALWAYS DELETE NAS CONTEXT FROM IMSI. This way we ensure that Identity request, Auth request and Security mode command are sent in every fuzzing iteration.
...
bool nas::handle_identity_response(srsran::byte_buffer_t* nas_rx)
{
  ...
  if (nas_ctx != nullptr) {
    ...
    // FUZZ START: EPC CRASH FIX. Make sure that we don't delete ourselves and hence not invalidate the pointer.
    if (nas_ctx != this) m_s1ap->delete_ue_ctx(imsi);
    // FUZZ END: EPC CRASH FIX
