...
// FUZZ (new reset function of all layers)
void ue_stack_nr::reset()
{
  printf("5G: START RESETTING LAYERS!\n");
  // mac->reset();
  // rlc->reset();
  // pdcp->reset();
  // //sdap->init(, gw);
  // // nas.init(usim.get(), &rrc, gw, args.nas);
  // rrc->init(phy, mac.get(), rlc.get(), pdcp.get(), sdap.get(), gw, nullptr, nullptr, nullptr, task_sched.get_timer_handler(), this, args.rrc_nr);
  // //sdap->init();
  // printf("5G: FINISHED RESETTING LAYERS!\n");
}
// FUZZ
...
{
  printf("5G: Switching ON the stack\n");
...
{
  printf("5G: Switching OFF the stack\n");
  return true;
}
