                                                   ...
                                                   mac_interface_phy_lte::tb_action_ul_t* action)
{
  ...
  if (grant.tb.ndi_present) {
    ...
    // FUZZ START: UL MAC INTERCEPTION
    // printf("UE Interception point 6");
    // print_bytes(pdu_ptr, grant.tb.tbs);
    if (fuzz_config_g.FUZZ_MAC_UL) {
        shm_helper_g.send_mac_msg(
          T_ENB_MAC_UE_UL_PDU_WITH_DATA,
          DIRECTION_UPLINK,
          WS_C_RNTI,
          grant.rnti, 
          (uint16_t)(grant.tti_tx/10),
          (uint16_t)(grant.tti_tx%10),
          pdu_ptr,
          grant.tb.tbs,
          -1
