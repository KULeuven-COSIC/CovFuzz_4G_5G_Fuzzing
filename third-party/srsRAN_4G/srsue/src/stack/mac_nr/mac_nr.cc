...
{
  // FUZZ
  printf("UE Interception point 4");
  print_bytes(tb.payload->msg, tb.payload->N_bytes);
...
void mac_nr::new_grant_ul(const uint32_t cc_idx, const mac_nr_grant_ul_t& grant, tb_action_ul_t* action)
{
  ...
  *action = {};
  ...
  // FUZZ
  printf("UE Interception point 5");
  print_bytes(action->tb.payload->msg, action->tb.payload->N_bytes);
