...
// FUZZ START: UE RESET FUNCTION
void ue_stack_lte::reset()
{
  printf("4G: START RESETTING LAYERS!\n");
  mac.reset();
  rlc.reset();
  pdcp.reset();
  ...
  printf("4G: FINISHED RESETTING LAYERS!\n");
}
// FUZZ END: UE RESET FUNCTION
...
bool ue_stack_lte::get_metrics(stack_metrics_t* metrics)
{
  ...
  ue_task_queue.try_push([this]() {
    ...
    nas_5g.get_metrics(metrics.nas_5g);
