...
void rrc::send_ul_ccch_msg(const ul_ccch_msg_s& msg)
{
  ...
  // FUZZ START: UL CCCH INTERCEPTION POINT 
  if (fuzz_config_g.FUZZ_RRC_UL) {
    shm_helper_g.send_rrc_msg(DIRECTION_UPLINK, CHANNEL::UL_CCCH, reinterpret_cast<uint8_t*>(pdcp_buf->msg), pdcp_buf->N_bytes);
  }
  // FUZZ END: UL CCCH INTERCEPTION POINT 
...
void rrc::send_ul_dcch_msg(uint32_t lcid, const ul_dcch_msg_s& msg)
{
  ...
  // FUZZ START: UL DCCH INTERCEPTION POINT 
  if (fuzz_config_g.FUZZ_RRC_UL) {
    shm_helper_g.send_rrc_msg(DIRECTION_UPLINK, CHANNEL::UL_DCCH, reinterpret_cast<uint8_t*>(pdcp_buf->data()), pdcp_buf->N_bytes);
  }
  // FUZZ END: UL DCCH INTERCEPTION POINT 
...
void rrc::parse_dl_ccch(unique_byte_buffer_t pdu)
{
  ...
  // FUZZ START: DL CCCH INTERCEPTION POINT 
  if (fuzz_config_g.FUZZ_RRC_UL) {
      shm_helper_g.send_rrc_msg(DIRECTION_DOWNLINK, CHANNEL::DL_CCCH, reinterpret_cast<uint8_t*>(pdu->msg), pdu->N_bytes);
  }
  // FUZZ END: DL CCCH INTERCEPTION POINT 
...
void rrc::parse_dl_dcch(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  // FUZZ START: DL DCCH INTERCEPTION POINT 
  if (fuzz_config_g.FUZZ_RRC_UL) {
      shm_helper_g.send_rrc_msg(DIRECTION_DOWNLINK, CHANNEL::DL_DCCH, reinterpret_cast<uint8_t*>(pdu->msg), pdu->N_bytes);
  }
  // FUZZ END: DL DCCH INTERCEPTION POINT 
  ...
  switch (dl_dcch_msg.msg.c1().type().value) {
    ...
    case dl_dcch_msg_type_c::c1_c_::types::security_mode_cmd:
      ...
      // FUZZ START: BUFFER OVERFLOW IN UE FIX
      if (sec_cfg.cipher_algo < sizeof(ciphering_algorithm_id_text)/sizeof(ciphering_algorithm_id_text[0])
       && sec_cfg.integ_algo < sizeof(integrity_algorithm_id_text)/sizeof(integrity_algorithm_id_text[0])) {
      ...
      // FUZZ END: BUFFER OVERFLOW IN UE FIX
