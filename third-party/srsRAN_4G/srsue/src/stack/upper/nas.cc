...
void nas::timer_expired(uint32_t timeout_id)
{
  if (timeout_id == t3402.id()) {
    ...
    if (attach_attempt_counter < max_attach_attempts) {
      ...
      // FUZZ START: SEND ATTACH FAILED TO FUZZER
      printf("SEND ATTACH FAILED TO THE FUZZER\n");
      if (!connected) {
        zmq_client.connect("tcp://127.0.0.1:5557");
        connected = true;
      }
      if (!zmq_client.send("ATTACH FAILED")) {
        printf("Failed to send ATTACH FAILED command to the fuzzer\n");
      } else {
        boost::optional<zmq::message_t> msg = zmq_client.recv();
        if (msg) {
          if (msg.value().to_string() == "OK") printf("Received answer OK\n");
          else printf("Received unexpected answer: %s from the fuzzer\n", msg.value().to_string().c_str());
        } else {
          printf("ERROR HAPPENED while receiving the message from the fuzzer\n");
        }
      }
      // FUZZ END: SEND ATTACH FAILED TO FUZZER
  ...
  if (attach_accept.eps_attach_result == LIBLTE_MME_EPS_ATTACH_RESULT_EPS_ONLY) {
    ...
    if (LIBLTE_MME_PDN_TYPE_IPV4 == act_def_eps_bearer_context_req.pdn_addr.pdn_type) {
      ...
      srsran::console("Network attach successful. IP: %u.%u.%u.%u\n",
                      ...
                      act_def_eps_bearer_context_req.pdn_addr.addr[2],
                      act_def_eps_bearer_context_req.pdn_addr.addr[3]);
      // FUZZ START: SEND ATTACH SUCCESSFUL TO FUZZER
      printf("SEND ATTACH SUCCESSFUL TO THE FUZZER\n");
      if (!connected) {
        zmq_client.connect("tcp://127.0.0.1:5557");
        connected = true;
      }
      if (!zmq_client.send("ATTACH SUCCESSFUL")) {
        printf("Failed to send ATTACH SUCCESSFUL command to the fuzzer\n");
      } else {
        boost::optional<zmq::message_t> msg = zmq_client.recv();
        if (msg) {
          if (msg.value().to_string() == "OK") printf("Received answer OK\n");
      ...
      // FUZZ END: SEND ATTACH SUCCESSFUL TO FUZZER
      // Setup GW
...
void nas::parse_security_mode_command(uint32_t lcid, unique_byte_buffer_t pdu)
{
  ...
  // FUZZ START: BUFFER OVERFLOW FIX
  if (sec_mode_cmd.selected_nas_sec_algs.type_of_eea < sizeof(ciphering_algorithm_id_text)/sizeof(ciphering_algorithm_id_text[0])
       && sec_mode_cmd.selected_nas_sec_algs.type_of_eia < sizeof(integrity_algorithm_id_text)/sizeof(integrity_algorithm_id_text[0])) {
