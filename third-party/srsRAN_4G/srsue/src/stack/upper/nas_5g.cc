...
{
  // TODO
  printf("TIMER %u HAS EXPIRED\n", timeout_id);
  // if (timeout_id == t3510.id()) {
  //     // FUZZ: SEND ATTACH FAILED TO FUZZER
  //     printf("5G: Sending ATTACH FAILED TO FUZZER\n");
  //     if (!connected) {
                            ...
                            srsran::nas_5g::pdu_address_t pdu_address)
{
  ...
  if (pdu_session_type == LIBLTE_MME_PDN_TYPE_IPV4V6 || pdu_session_type == LIBLTE_MME_PDN_TYPE_IPV4) {
    ...
    // FUZZ: SEND ATTACH SUCCESSFUL TO FUZZER
    // if (!connected) {
    //   zmq_client.connect("tcp://127.0.0.1:5557");
    //   connected = true;
    // }
