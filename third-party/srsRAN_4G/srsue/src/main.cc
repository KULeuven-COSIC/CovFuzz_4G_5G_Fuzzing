...
// FUZZ INCLUDE FUZZ_CONFIG
#include "fuzz_config.h"

// FUZZ EXTERN FUZZ_CONFIG
extern Fuzz_Config fuzz_config_g;

// FUZZ DECLARE KEEP_RUNNING
...
int main(int argc, char* argv[])
{
  ...
  // FUZZ START: INIT SHARED MEMORY
  if (!parse_fuzz_config()) return -1;
  SHM::keep_running = 1;
  //keep_running = 1;
  //if (fuzz_config_g.fuzz_ul()) init_shm(SHM_CLIENT, SHM_SIZE, &keep_running);
  // FUZZ END: INIT SHARED MEMORY
  ...
  // FUZZ START: COMMUNICATION WITH THE FUZZER
  cout << endl << "Initializing UE's socket...";
  ZMQ_Server zmq_server(true);
  if (!zmq_server.bind("tcp://*:5556")) {
    cout << endl << "Couldn't initialize the socket: exiting..." << endl;
    exit(0);
  }
  cout << "done" << endl << endl;
  ...
  while (running) {
    ...
    if (msg)
    {
      std::string cmd = msg.value().to_string();
      cout << endl << endl <<  "Received new request: " << cmd << endl;

      // Take action depending on what is received
      //if (string(buf).substr(0, 13) == "disconnected?")
      if (cmd == "disconnected?")
      {
        srsue::ue_metrics_t metr;
        ue.get_metrics(&metr);
        if (metr.stack.rrc.state == RRC_STATE_IDLE)
        {
          // Send that the UE is disconnected from the network
          //zmq_send(responder, "OK", 2, 0);
          if (!zmq_server.send("OK")) cout << "Failed to send OK\n\n";
          cout << "UE is disconnected from the network... " << endl << endl << endl;
        } else
        {
          // Send that the UE is connected to the network or not yet finished with processing
          if (!zmq_server.send("")) cout << "Failed to send\n\n";
          //zmq_send(responder, "", 0, 0);
          cout << "UE's state is undefined... " << endl << endl << endl;
        }
      } else if (cmd == "connected?") {
        cout << "Checking if UE is connected to the network\n";
        srsue::ue_metrics_t metr;
        ue.get_metrics(&metr);
        if (metr.stack.nas_5g.state == mm5g_state_t::state_t::registered)
        {
          // Send that the UE is connected to the network
          //zmq_send(responder, "OK", 2, 0);
          cout << "UE is connected to the network... " << endl << endl << endl;
          if (!zmq_server.send("OK")) cout << "Failed to send OK\n\n";
        } else
        {
          // Send that the UE is not connected to the network or not yet finished with processing
          //zmq_send(responder, "", 0, 0);
          cout << "UE's state is undefined... " << endl << endl << endl;
          if (!zmq_server.send("")) cout << "Failed to send\n\n";
        }
      } else if (cmd == "connect") {
        // Send an ACK result back
        //zmq_send(responder, "ACK", 3, 0);
        if (!zmq_server.send("OK")) cout << "Failed to send OK\n\n";
        cout << "Connect UE to the network... ";
        while (!ue.switch_on())
        {
          cout << endl << "retrying...";
          sleep(2);
        ...
        if (metr.stack.nas_5g.state == mm5g_state_t::state_t::registered)
        {
          cout << "UE is connected to the network... " << endl;
        } else
        {
          cout << "UE's state is undefined... " << endl;
        }
      } else if (cmd == "disconnect") {
        // Send an ACK result back
        //zmq_send(responder, "ACK", 3, 0);
        if (!zmq_server.send("OK")) cout << "Failed to send OK\n\n";
        cout << "Disconnect UE from the network... ";
        while (!ue.switch_off())
        {
          cout << endl << "retrying...";
          sleep(1);
        ...
        zmq_server.send(""); // Send smth to avoid deadlock
        cout << "Send smth to avoid deadlock\n\n";
      }
    }
  }
  //print_total_triggered_edges();
  // FUZZ START: COMMUNICATION WITH THE FUZZER
