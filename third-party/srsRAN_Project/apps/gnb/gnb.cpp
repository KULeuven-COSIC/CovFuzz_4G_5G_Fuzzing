...
// FUZZ: INCLUDE COVERAGE INSTRUMENTATION
...
// FUZZ
// if (fuzz_config.INTERCEPT_ON) {

//   std::thread(zmq_cov::worker_loop).detach();
//   if (parse_fuzz_config()) return -1;
//   keep_running = 1;
//   if (fuzz_config.FUZZ_DL) init_shm(SHM_CLIENT, SHM_SIZE, &keep_running);
// }
// FUZZ END
  ...
  SHM::keep_running = 1;
  //keep_running = 1;
  //if (fuzz_config_g.fuzz_dl()) init_shm(SHM_CLIENT, SHM_SIZE, &keep_running);
