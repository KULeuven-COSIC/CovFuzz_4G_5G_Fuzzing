...
// FUZZ INCLUDE IOSTREAM IOMANIP
// #include <iostream>
// #include <iomanip>
// FUZZ END INCLUDE IOSTREAM IOMANIP
  ...
  for (const dl_msg_alloc& grant : dl_res.ue_grants) {
    for (unsigned cw_idx = 0; cw_idx != grant.pdsch_cfg.codewords.size(); ++cw_idx) {
      ...
      if (cw.new_data) {
        ...
        // FUZZ START: DL MAC INTERCEPTION 1
        if (fuzz_config_g.FUZZ_MAC_DL) {
                uint32_t size = pdu.size();
                uint8_t* pdu_data = const_cast<uint8_t*>(pdu.data());
                shm_helper_g.send_mac_msg_nr(0,
                      DIRECTION_DOWNLINK,
                      NR_C_RNTI,
                      grant.pdsch_cfg.rnti,
                      (uint16_t)sl_tx.sfn(),
                      (uint16_t)sl_tx.slot_index(),
                      pdu_data,
                      size,
                      -1);
        }
        // FUZZ END: DL MAC INTERCEPTION 1
      } else {
        pdu =
