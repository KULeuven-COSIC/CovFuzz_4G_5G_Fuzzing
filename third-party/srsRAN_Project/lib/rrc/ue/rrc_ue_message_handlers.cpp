...
if (fuzz_config_g.FUZZ_RRC_DL) {
  ...
  shm_helper_g.send_rrc_msg(DIRECTION_UPLINK, CHANNEL::UL_CCCH, reinterpret_cast<uint8_t*>(&(pdu[0])), pdu_length);
}
// FUZZ END: RRC_UL_CCCH interception
...
// FUZZ: RRC_UL_DCCH interception
if (fuzz_config_g.FUZZ_RRC_DL) {
  rrc_pdu.linearize(); // Make sure that the byte buffer is contineous in memory
  uint32_t pdu_length = (uint32_t)rrc_pdu.length();
  shm_helper_g.send_rrc_msg(DIRECTION_UPLINK, CHANNEL::UL_DCCH, reinterpret_cast<uint8_t*>(&(rrc_pdu[0])), pdu_length);
}
// FUZZ END: RRC_UL_DCCH interception
